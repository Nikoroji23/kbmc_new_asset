...
";
    ...
    public function connect($host, $port = null, $timeout = 30, $options = []) {
        ...
        if (null === $port) $port = self::DEFAULT_PORT;
        ...
        $this->smtp_conn = @stream_socket_client($host . ':' . $port, $errno, $errstr, $timeout, STREAM_CLIENT_CONNECT, stream_context_create($options));
        if (!is_resource($this->smtp_conn)) {
            $this->error = ['error' => 'Failed to connect to server', 'detail' => "$errstr ($errno)", 'smtp_code' => '', 'smtp_code_ex' => ''];
    ...
    public function authenticate($username, $password, $authtype = null, $OAuth = null) {
        if (!$this->server_caps) $this->setError('Authentication is not allowed before HELO/EHLO');
        if (array_key_exists('EHLO', $this->server_caps)) {
            ...
            if (null !== $authtype && !in_array($authtype, $this->server_caps['AUTH'])) {
        ...
        } elseif (null == $authtype) $authtype = 'LOGIN';
        $this->edebug('Auth method selected: ' . $authtype, self::DEBUG_LOWLEVEL);
        switch ($authtype) {
            case 'PLAIN':
                if (!$this->sendCommand('AUTH', 'AUTH PLAIN', 334)) return false;
                if (!$this->sendCommand('User & Password', base64_encode("" . $username . "" . $password), 235)) return false;
                break;
            case 'LOGIN':
            ...
            case 'CRAM-MD5':
                ...
                $response = $username . ' ' . $this->hmac($password, $challenge);
...
", $msg_data));
        ...
        foreach ($lines as $line) {
            ...
            while (strlen($line) > $maxlen) {
                $pos = strrpos(substr($line, 0, $maxlen), ' ');
                if (!$pos) {
                    $pos = $maxlen - 1;
    ...
    public function hello($host = '') {
        if ($this->sendCommand('HELO', "HELO $host", 250)) return true;
        if ($this->sendCommand('EHLO', "EHLO $host", 250)) {
            if ($this->server_caps) return true;
        }
        return false;
    }
    ...
    public function recipient($to, $dsn = '') {
        if (empty($dsn)) $rcpt = 'RCPT TO:<' . $to . '>';
            ...
            $rcpt = 'RCPT TO:<' . $to . '> NOTIFY=' . implode(',', $notify);
        }
    ...
    protected function get_lines() {
        ...
        while (is_resource($this->smtp_conn) && !feof($this->smtp_conn)) {
            ...
            if ($info['timed_out']) {
                $this->edebug('SMTP -> get_lines(): timed-out (' . $this->Timelimit . ' sec)', self::DEBUG_LOWLEVEL);
                break;
            }
    ...
    protected function recordLastTransactionID() {
        $reply = $this->getLastTransactionID();
        if (empty($reply)) $this->last_smtp_transaction_id = null;
        else $this->last_smtp_transaction_id = $reply;
    }

    protected function getSMTPConnection($host, $port = null, $timeout = 30, $options = []) {
        return $this->connect($host, $port, $timeout, $options);
    }
    ...
    protected function sendCommand($command, $commandstring, $expect) {
        ...
        $matches = [];
        if (preg_match('/^(\d{3})[ -](?:(\d\.\d\.\d) )?/', $this->last_reply, $matches)) {
            $code = $matches[1];
            $code_ex = (count($matches) > 2 ? $matches[2] : null);
            $detail = preg_replace("/{$code}[ -]" . ($code_ex ? str_replace('.', '\.', $code_ex) . ' ' : '') . '/ms', '', $this->last_reply);
        } else {
        ...
        $this->edebug('SERVER -> CLIENT: ' . $code . ' ' . $detail, self::DEBUG_SERVER);
        if (!in_array($code, (array)$expect, true)) {
            $this->setError($command . ' command failed', $detail, $code, $code_ex);
            $this->edebug('SMTP ERROR: ' . $this->error['error'] . ': ' . $detail, self::DEBUG_CLIENT);
            return false;
        }
        $this->setError('');
        return true;
